...
test.describe('일기쓰기 모달 닫기 기능', () => {
  ...
  test('닫기 버튼 클릭 시 등록취소 모달이 표시되어야 함', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="modal-content"]')).toBeVisible({ timeout: 500 });
    
    // 등록취소 모달의 제목과 내용 확인
    await expect(page.locator('[data-testid="modal-content"] h2')).toHaveText('등록을 취소하시겠습니까?');
    await expect(page.locator('[data-testid="modal-content"] p')).toHaveText('작성 중인 내용이 사라집니다.');
  ...
  test('계속작성 버튼 클릭 시 등록취소 모달만 닫혀야 함', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="modal-content"]')).not.toBeVisible({ timeout: 500 });
  ...
  test('등록취소 모달의 variant와 actions가 올바르게 설정되어야 함', async ({ page }) => {
    ...
    const modalElement = page.locator('[data-testid="modal-content"]');
